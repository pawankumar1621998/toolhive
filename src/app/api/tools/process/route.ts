  ...
  switch (slug) {
    ...
    case "translate-pdf": {
      try {
        ...
        const result  = header + translated.trim();
      ...
      for (let fi = 0; fi < bufs.length; fi++) {
        try {
          ...
          for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
            try {
              const pageResults = await convert.bulk([pageNum]);
