...
import { Upload, X, Download, RotateCcw, CheckCircle2, FileText, Plus } from "lucide-react";
...
function PDFPreviewCard({
  ...
  return (
    <div className="group relative bg-background rounded-xl border border-border overflow-hidden hover:border-orange-400/40 hover:shadow-md transition-all duration-200">
      {/* Thumbnail placeholder */}
      <div className="relative bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-950/20 dark:to-amber-950/20 aspect-square overflow-hidden flex items-center justify-center">
        <FileText className="h-16 w-16 text-orange-400 dark:text-orange-500/60" />
        {/* Page count badge */}
        {fwp.pageCount && (
  ...
  return (
    <div className="bg-background rounded-xl border border-border overflow-hidden hover:border-emerald-400/50 hover:shadow-sm transition-all group">
      {isImage ? (
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/20 dark:to-teal-950/20 aspect-video flex items-center justify-center">
          <FileText className="h-12 w-12 text-emerald-500/60" />
        </div>
      )}
...
// ─── Get page count from PDF (client-side) ────────────────────────────────────
        ...
        <div
          ...
          className={clsx(
            ...
            isDragging
              ...
              : "border-border hover:border-orange-400 hover:bg-background-subtle"
          )}
        >
          <input
            ref={inputRef}
          ...
          <div className="flex items-center justify-center mb-5">
            <div className={clsx(
              ...
              isDragging ? "bg-orange-100 dark:bg-orange-900/40" : "bg-background-subtle border border-border"
            )}>
              <FileText className={clsx("h-8 w-8", isDragging ? "text-orange-600" : "text-foreground-muted")} />
            </div>
          </div>

          <p className="text-lg font-semibold text-foreground mb-1.5">
            {isDragging ? "Drop PDFs here" : "Select Or Drag & Drop PDFs Here"}
          </p>
          <p className="text-sm text-foreground-muted mb-6">
            {accept === ".pdf" ? "PDF files only" : accept.replace(/\./g, "").toUpperCase()}
            {" · "}Max {tool.maxFileSizeMB > 0 ? `${tool.maxFileSizeMB} MB` : "100 MB"} per file
          </p>

          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); inputRef.current?.click(); }}
            className="inline-flex items-center gap-2 bg-orange-600 hover:bg-orange-700 active:bg-orange-800 text-white px-8 py-3 rounded-xl text-sm font-semibold transition-colors shadow-sm"
          >
          ...
          {maxFiles > 1 && (
            <p className="mt-4 text-xs text-foreground-subtle">
              You can select up to {maxFiles} PDFs at once
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-start">
        ...
        <div className="flex-1 min-w-0">
          ...
          >
            {/* Drag overlay */}
            {isDragging && (
              <div className="absolute inset-0 z-20 rounded-2xl flex items-center justify-center bg-orange-50/80 dark:bg-orange-950/40 border-2 border-orange-500 border-dashed">
                <div className="text-center">
                  <Upload className="h-8 w-8 text-orange-600 mx-auto mb-2" />
                  <p className="text-sm font-semibold text-orange-700 dark:text-orange-400">Drop to add PDFs</p>
                </div>
              </div>
            )}
            ...
            {processing && (
              <div className="absolute inset-0 z-20 rounded-2xl flex flex-col items-center justify-center bg-background/80 backdrop-blur-sm">
                <div className="h-10 w-10 rounded-full border-[3px] border-border border-t-orange-600 animate-spin mb-3" />
            ...
            <div className="p-3">
              ...
              <div className="flex items-center justify-between mb-3 px-1">
                <span className="text-xs font-semibold text-foreground-muted uppercase tracking-wide">
                  {filesWithPreviews.length} PDF{filesWithPreviews.length > 1 ? "s" : ""} Selected
                </span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                ...
                {filesWithPreviews.length < maxFiles && (
                  <button
                    type="button"
                    onClick={() => addMoreRef.current?.click()}
                    className="aspect-square rounded-xl border-2 border-dashed border-border hover:border-orange-400 hover:bg-orange-50/30 dark:hover:bg-orange-950/10 flex flex-col items-center justify-center gap-1.5 text-foreground-muted hover:text-orange-600 transition-all"
                  >
              ...
              {maxFiles > 1 && (
                <p className="text-[10px] text-foreground-subtle mt-3 px-1">
                  {filesWithPreviews.length}/{maxFiles} PDFs
                </p>
              )}
            </div>
          </div>
