...
interface FormState {
  ...
  companyName: string; hiringManager: string; jobDescription: string; tone: Tone;
}

export function CoverLetterGen() {
  const [form, setForm] = useState<FormState>({
    fullName: "", email: "", jobTitle: "", companyName: "",
    hiringManager: "", jobDescription: "", tone: "Professional",
  });
  const [letter, setLetter] = useState<string | null>(null);
  const [copied, setCopied]  = useState(false);
  const { generate, loading, error, output } = useAIGenerate("cover-letter");
  ...
  const handleGenerate = () => generate({
    name: form.fullName, jobTitle: form.jobTitle, company: form.companyName,
    experience: form.jobDescription, tone: form.tone,
    ...(form.hiringManager && { hiringManager: form.hiringManager }),
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className={cardClass}>
        ...
        <p className="text-sm text-foreground-muted mb-6">Fill in your details and generate a personalized AI cover letter.</p>
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><label className={labelClass}>Full Name</label><input type="text" value={form.fullName} onChange={set("fullName")} placeholder="Jane Smith" className={inputClass} /></div>
            <div><label className={labelClass}>Email</label><input type="email" value={form.email} onChange={set("email")} placeholder="jane@example.com" className={inputClass} /></div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><label className={labelClass}>Job Title</label><input type="text" value={form.jobTitle} onChange={set("jobTitle")} placeholder="Product Manager" className={inputClass} /></div>
            <div><label className={labelClass}>Company Name</label><input type="text" value={form.companyName} onChange={set("companyName")} placeholder="Acme Corp" className={inputClass} /></div>
          </div>
          <div><label className={labelClass}>Hiring Manager <span className="text-foreground-muted font-normal">(optional)</span></label><input type="text" value={form.hiringManager} onChange={set("hiringManager")} placeholder="John Doe" className={inputClass} /></div>
          <div><label className={labelClass}>Paste Job Description</label><textarea value={form.jobDescription} onChange={set("jobDescription")} placeholder="Paste the full job description here…" rows={6} className={textareaClass} /></div>
          <div>
          ...
          <Button variant="gradient" fullWidth isLoading={loading} loadingText="Generating…" onClick={handleGenerate} disabled={!isValid} className="bg-gradient-to-r from-indigo-500 to-purple-600 mt-2">Generate Cover Letter</Button>
